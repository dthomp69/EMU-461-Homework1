...
public class Knapsack {
	...
	public void setParameters(int populationSize, int stringLength, int numberIterations, double crossoverRate,
	...
	private void display(int[] string) {
		...
		int sumBuyingPrice = 0;
		int sumValue = 0;
		// go thru string
		for (int i = 0; i < stringLength; i++) {
			// if item is selcted print its weight and value
			if (string[i] == 1) {
				System.out.println(table[i][0] + " " + table[i][1]);
				sumBuyingPrice += table[i][0];
				sumValue += table[i][1];
			}
		}
		System.out.println("Sum buying price: " + sumBuyingPrice);
